...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button type="submit" name="add_prod_out_sale"  class="btn btn-primary">Add plant</button>
      </div>
      </form>
    </div>
  </div>
  ...
  <button id="Add_plant" class="btn btn-primary" data-toggle="modal" data-target="#exampleModal">
    ...
    <div class="card">
              <div class="card-header">
                <h3 class="card-title">On Sale Outdoor Plants</h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
  ...
  <?php
   ...
   require_once "dbc.php";
         ...
         $sql="SELECT * FROM `prod_search_sale`";
         ...
         while($row = mysqli_fetch_assoc($result)){
              ...
              <td>".$row['prnamee']."</td>
              <td>".$row['primgg']."</td>
              <td>".$row['prod_descc']."</td>
              <td>".$row['prpricee']."</td>
              <td>".$row['prdelpricee']."</td>
              
              <form action='edittoutsale.php' method='post'>
              ...
              <td id='gotcha'><input type='hidden' name='hps_eprod_id' value='".$row['pridd']."' ></td>
              </form>

              <form action='Upload.php' method='post'>
              <td clsss='btn_conta'>
              <input class='btn btn-primary float-left bg-red' type='submit' name='sdlt_proh_one_sale' value='Delete' onclick='return arSureOnepro()'></td>
              <td><input id='gotcha' type='hidden' name='hp_prod_id' value='".$row['pridd']."' ></td>
              </tr>
              </form>";
              $srno++;
         }
