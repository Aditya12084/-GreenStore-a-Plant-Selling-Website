...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button type="submit" name="add_prod_house"  class="btn btn-primary">Add plant</button>
      </div>
      </form>
    </div>
  </div>
  ...
  <button id="Add_plant" class="btn btn-primary" data-toggle="modal" data-target="#exampleModal">
    ...
    <div class="card">
              <div class="card-header">
                <h3 class="card-title">Regular Price House Plants</h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
  ...
  <?php
   ...
   require_once "dbc.php";
         ...
         $sql="SELECT * FROM `prod_sch_house`";
         ...
         while($row = mysqli_fetch_assoc($result)){
              ...
              <td id='gotcha'><input type='hidden' name='hp_eprod_id' value='".$row['hp_id']."' ></td>
              </form>

              <form action='Upload.php' method='post'>
              <td clsss='btn_conta'>
              <input class='btn btn-primary float-left bg-red' type='submit' name='dlt_proh_one' value='Delete' onclick='return arSureOnepro()'></td>
              <td><input id='gotcha' type='hidden' name='hp_prod_id' value='".$row['hp_id']."' ></td>
              </tr>
              </form>";
              $srno++;
         }
